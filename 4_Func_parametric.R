...
Func_parametric <- function(gap.obs){ 
            ...
            init <- 1 #quantile(gap.obs, 0.5, na.rm = T) #quantile(gap.obs, 0.05)
            
            M.model <- optim(par = c(init, 0, 1), f.beta , gap.obs = gap.obs , type = "M")
            L.model <- optim(par = c(init, 0, 1), f.beta , gap.obs = gap.obs , type = "L")
            G.model <- optim(par = c(init, 0, 1), f.beta , gap.obs = gap.obs , type = "G")
